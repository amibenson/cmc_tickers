...
def print_ticker_history_rs_data(rs_TickerHistory, alert_trading_volume_percent_th = None):
    ...
    if rs:
        ...
        for reading in rs:
            ...
            print("%s symbol ticker was read %s, value %s BTC (%s%% daily change) with %s trading percent (MCAP: %s)" % \
                  (reading.symbol, get_time_ago(reading.lastUpdated), reading.priceBtc, reading.percentChange24h, s_percent, format_using_humanize(reading.markedCapUsd, humanize.intword)) \
                  )
